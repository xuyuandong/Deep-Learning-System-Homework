...
def softmax_loss(Z, y_one_hot):
    ...
    """
    ### BEGIN YOUR SOLUTION
    losses = ndl.log(ndl.summation(ndl.exp(Z), axes=1)) - ndl.summation(Z * y_one_hot, axes=1)
...
def nn_epoch(X, y, W1, W2, lr = 0.1, batch=100):
    ...
    while start < total:
        ...
        bx = X[start:end]
        by = np.zeros((end - start, classes))
        by[np.arange(end - start), y[start:end]] = 1

        BX = ndl.Tensor(bx)
        BY = ndl.Tensor(by)
        BZ = ndl.relu(BX @ W1) @ W2

        loss = softmax_loss(BZ, BY)
        #print(loss)
        ...
        W1 = ndl.Tensor(W1.numpy() - lr*gw1)
        W2 = ndl.Tensor(W2.numpy() - lr*gw2)
