...
def gradient_check(f, *args, tol=1e-6, backward=False, **kwargs):
    ...
    #print('---------', out.shape, computed_grads[0].shape, numerical_grads[0].shape)
    #print(computed_grads[0])
...
def test_broadcast_to_backward():
    ...
    gradient_check(ndl.broadcast_to, ndl.Tensor(np.random.randn(1, 3)), shape=(3, 3, 3)) ## my add
    gradient_check(ndl.broadcast_to, ndl.Tensor(np.random.randn(3, 1)), shape=(3, 3, 3)) ## my add
