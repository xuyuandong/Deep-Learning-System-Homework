...
def test_rnn_cell(batch_size, input_size, hidden_size, bias, init_hidden, nonlinearity, device):
    ...
    print('torch', h_)
    print('torch bias', model_.bias_ih, model_.bias_hh)
    ...
    print('RNN-Wih', model.W_ih)
    print('RNN-Whh', model.W_hh)
    print('--bih', model.bias_ih)
    print('--bhh', model.bias_hh)
    print('to set bias', bias)
    if bias:
        print('set bias')
    ...
    print('RNN-bih', model.bias_ih)
    print('RNN-bhh', model.bias_hh)
    if init_hidden:
...
def test_language_model_training(device):
    ...
    if str(device) == "cpu()":
        ...
        print('ERROR', str(device))


@pytest.mark.parametrize("device", _DEVICES, ids=["cpu", "cuda"])
def test_language_model_training_lstm(device):
    corpus = ndl.data.Corpus("data/ptb", max_lines=20)
    seq_len = 8
    num_examples = 88
    batch_size = 12
