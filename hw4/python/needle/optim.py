...
class SGD(Optimizer):
    ...
    def step(self):
        ...
        for p in self.params:
            if p not in self.u:
                self.u[p] = ndl.init.zeros(*p.shape, device=p.device, dtype=p.dtype, requires_grad=False)
...
class Adam(Optimizer):
    ...
    def step(self):
        ...
        for p in self.params:
            if p not in self.m:
                self.m[p] = ndl.init.zeros(*p.shape, device=p.device, dtype=p.dtype, requires_grad=False)
                self.v[p] = ndl.init.zeros(*p.shape, device=p.device, dtype=p.dtype, requires_grad=False)
