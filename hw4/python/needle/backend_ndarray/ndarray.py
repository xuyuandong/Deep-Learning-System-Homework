...
class NDArray:
    ...
    def reshape(self, new_shape):
        ...
        if prod(self._shape) != prod(new_shape):
            print('reshape shape=', self._shape, 'newshape=', new_shape)
            raise ValueError()
    ...
    def broadcast_to(self, new_shape):
        ...
        return NDArray.make(new_shape, strides=tuple(new_strides), device=self.device, handle=self._handle)
        ### END YOUR SOLUTION
    ...
    def __getitem__(self, idxs):
        ...
        new_offset = sum([idx.start * self.strides[i] for i, idx in enumerate(idxs)])
    ...
    def flip(self, axes):
        ...
        """
        ### BEGIN YOUR SOLUTION
        newstrides = [-s if i in axes else s for i,s in enumerate(self.strides)]
        #print('newstride', newstrides)
        offset = 0
        for i in axes:
            offset += (self.shape[i] - 1) * self.strides[i]
        #print('newoffset', offset)
        result = NDArray.make(self.shape, strides=newstrides, device=self.device, handle=self._handle, offset=offset)
    ...
    def pad(self, axes):
        ...
        """
        ### BEGIN YOUR SOLUTION
        newshape = [s + axes[i][0] + axes[i][1] for i, s in enumerate(self.shape)]
        idxs = [slice(axes[i][0], axes[i][0]+s) for i,s in enumerate(self.shape)]
        result = NDArray.make(tuple(newshape), device=self.device)
        result.fill(0)
        result[tuple(idxs)] = self
        return result
        ### END YOUR SOLUTION
