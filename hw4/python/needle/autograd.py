...
def node_print_debug(node):
    if isinstance(node, Tensor):
        print(node.op, node.shape, node.dtype, node.grad.dtype)
    else:
        print(node.op)
...
def compute_gradient_of_variables(output_tensor, out_grad):
    ...
    for i, node in enumerate(reverse_topo_order):
        node.grad = reduce(lambda x, y: x + y, node_to_output_grads_list[node])
        #node_print_debug(node)

        if node.is_leaf():
            continue
        ginputs = node.op.gradient(node.grad, node)
        if isinstance(ginputs, tuple):
            ginputs = list(ginputs)
        else:
            ginputs = [ginputs]

        for k, vk in enumerate(node.inputs):
            gk = ginputs[k]
            if vk not in node_to_output_grads_list:
                node_to_output_grads_list[vk] = [gk]
            else:
                node_to_output_grads_list[vk].append(gk)
    #print('<----backward')
    ### END YOUR SOLUTION
